...
public class WallTileImageProvider implements TileImageProvider {
	...
	public WallTileImageProvider(SpriteSheet sheet, int frames) {
		this.sheet = sheet;
		this.frames = frames;
	}
	...
	public int tilesWide() {
		return sheet.tilesWide() / 8 / frames;
	}
	...
	private void getConnectedTile(int tileX, int tileY, int tl, int tr, int bl, int br, BufferedImage[] images) {
		int x = tileX*8*frames + 4 + (int)(LevelEditor.tick % frames)*frames;
		int x2 = tileX*8*frames + (int)(LevelEditor.tick % frames)*frames;
		...
		images[4] = sheet.get(x2 + offX[UP_LEFT][tl], tileY*6 + offY[UP_LEFT][tl]);
		images[5] = sheet.get(x2 + offX[UP_RIGHT][tr], tileY*6 + offY[UP_RIGHT][tr]);
		images[6] = sheet.get(x2 + offX[DOWN_LEFT][bl], tileY*6 + offY[DOWN_LEFT][bl]);
		images[7] = sheet.get(x2 + offX[DOWN_RIGHT][br], tileY*6 + offY[DOWN_RIGHT][br]);
	}
